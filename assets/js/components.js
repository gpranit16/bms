...
function initializeNavbarBehavior() {
    ...
    const forceSolidNav = document.body.classList.contains('force-solid-nav');
    ...
    const onScroll = () => {
        const shouldUseSolid = forceSolidNav || window.scrollY > 50;

        if (shouldUseSolid && !isScrolled) {
            ...
            isScrolled = true;
        } else if (!shouldUseSolid && isScrolled) {
